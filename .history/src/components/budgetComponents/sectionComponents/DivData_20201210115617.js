...
const DivData = ({data, label, total, onClick}) => {
    const sign = data.category > 0 ? '+' : '-';
    return (
        <Container>
            {data.map(item => (
                <DivWrapper key={item.id}>
                    <IcoP>{item.category}</IcoP>
                    <NameH3>{item.name}</NameH3>
                    <AmountSpan>{sign}{item.amount}zł</AmountSpan>
                    <FontAwsomeP ><FontAwesomeIcon icon={faTrashAlt}/></FontAwsomeP>
                </DivWrapper>)) }
                <TotalAmountDiv label={label}><p>Total amount:</p>  <AmountP>{sign}{total}zł</AmountP></TotalAmountDiv>
        </Container>
    )
}
