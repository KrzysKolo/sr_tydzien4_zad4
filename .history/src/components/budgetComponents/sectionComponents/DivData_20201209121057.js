...
import { faTrashAlt, faMoneyBillWave} from '@fortawesome/free-solid-svg-icons';
        ...
        <Container>
            {data.map(item => (
                <DivWrapper key={item.id}>
                    <IcoP cat={item.category}>{item.category}<FontAwesomeIcon icon={faMoneyBillWave}/></IcoP>
                    <NameH3>{item.name}</NameH3>
                    <AmountSpan>{item.amount}zł</AmountSpan>
                    <FontAwsomeP  onClick={() => onClick(data.id)}><FontAwesomeIcon icon={faTrashAlt}/></FontAwsomeP>
                </DivWrapper>)) }
