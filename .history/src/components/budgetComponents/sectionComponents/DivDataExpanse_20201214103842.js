...
const DivData = ({label, expenseTransactions, total}) => {
        ...
        <Container>
            {expenseTransactions.map(item => (
                <DivWrapper key={item.id}>
                    <IcoP>{item.category > 0 ? 'ico' : <FontAwesomeIcon icon={faMoneyBillWave} />}</IcoP>
                    <NameH3>{item.name}</NameH3>
                    <AmountSpan>{item.category > 0 ? '-' : ''}{item.amount}zł</AmountSpan>
                    <FontAwsomeP onClick={() => deleteTransaction(item.id)}>{item.id}<FontAwesomeIcon icon={faTrashAlt}/></FontAwsomeP>
                </DivWrapper>)) }
