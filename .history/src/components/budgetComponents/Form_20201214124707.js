...
const FormData = ({ showForm, setShowForm }) => {
    ...
    const [income, setIncome] = useState({validationSchema});
    const [expense, setExpanse] = useState({name: "",
    amount: "",
    category: -1,});
    const onSubmit = (e, values, {resetForm}, {initialValues} ) => {
        console.log('Form data', values);
        e.preventDefault();
        console.log(initialValues.category);
        if (initialValues.category > 0 ){
            const newExpanseTransaction = ({
            id: uuidv4(),
            name: initialValues.name,
            amount: initialValues.amount*1,
            category: initialValues.category,
        });
        addExpense(newExpanseTransaction);
        setExpanse({
            name: "",
            amount: "",
            category: -1,
